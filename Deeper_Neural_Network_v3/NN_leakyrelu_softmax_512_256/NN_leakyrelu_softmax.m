...
hidden_layer_1_size = 512;   % 512 hidden units
...
% Save Model Weights
%save('NN_leakyrelu_softmax_weights.mat', 'Theta1', 'Theta2', 'Theta3');
...
y_train_pred = predict(Theta1, Theta2, Theta3, X_train);
fprintf('\nTraining Set Accuracy: %.2f %%\n', mean(double(y_train_pred == y_train)) * 100);

y_val_pred = predict(Theta1, Theta2, Theta3, X_val);
fprintf('\nValidation Set Accuracy: %.2f %%\n', mean(double(y_val_pred == y_val)) * 100);

y_test_pred = predict(Theta1, Theta2, Theta3, X_test);
