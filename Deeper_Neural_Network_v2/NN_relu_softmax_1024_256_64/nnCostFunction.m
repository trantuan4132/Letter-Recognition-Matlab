...
% Reshape nn_params back into the parameters Theta1, Theta2, Theta3, Theta4, 
...
a4 = relu(z4);
a4 = [ones(m, 1) a4];
a5 = softmax(a4 * Theta4');

% One-hot Encoding
y_n = bsxfun(@eq, y(:), 1:num_labels);

% Compute cost with regularization
J = -1/m * sum(sum(y_n .* log(a5)));
...
delta5 = a5 - y_n;
delta4 = (delta5 * Theta4) .* reluGradient([ones(m, 1) z4]);
delta4 = delta4(:, 2:end);
delta3 = (delta4 * Theta3) .* reluGradient([ones(m, 1) z3]);
delta3 = delta3(:, 2:end);
delta2 = (delta3 * Theta2) .* reluGradient([ones(m, 1) z2]);
delta2 = delta2(:, 2:end);

Theta4_grad = Theta4_grad + delta5' * a4;
